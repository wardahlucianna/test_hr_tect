...
namespace Rijndael256 {
  ...
  public class Rijndael {
    ...
    /// <param name="keySize">The cipher key size. 256-bit is stronger, but slower.</param>
    public static byte[] EncryptByte(byte[] plaintext, string password, KeySize keySize) {
      ...
      using (var msoutput = new MemoryStream()) {
        ...
        // Create a CryptoStream to encrypt the plaintext
        using (var cs = new CryptoStream(msoutput, CreateEncryptor(password, iv, keySize), CryptoStreamMode.Write)) {
          ...
          using (var msinput = new MemoryStream(plaintext)) {
            ...
            byte[] cipherTextBytes = msoutput.ToArray();

            // Close both streams.
            msoutput.Close(); cs.Close();
    ...
    /// <param name="keySize">The size of the cipher key used to create the ciphertext.</param>
    /// <returns>The plaintext.</returns>
    public static byte[] DecryptByte(byte[] ciphertext, string password, KeySize keySize) {
      // Open the ciphertext file
      using (var msinput = new MemoryStream(ciphertext,true)) {
        ...
        using (var msoutput = new MemoryStream()) {
          // Create a CryptoStream to decrypt the ciphertext
          using (var cs = new CryptoStream(msoutput, CreateDecryptor(password, iv, keySize), CryptoStreamMode.Write)) {
            // Create a buffer to process the plaintext file in chunks
            // Reading the whole file into memory can cause 
            // Out of Memory exceptions if the file is large
            var buffer = new byte[4096];
            ...
            //flush last final block
            cs.FlushFinalBlock();

            // Convert our encrypted data from a memory stream into a byte array.
            byte[] plainTextBytes = msoutput.ToArray();
